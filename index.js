...
class TwitterDumper {
	...
	static handleWorker(worker, i, resolve, reject) {
		...
		worker.on("message", (message) => {
			...
			if (message.info) {
				if (message.type !== "yok") fs.appendFileSync(`avail.txt`, message.username + "\n");
			}
		});
	...
	async dumpTwitter(username) {
		...
		if (body.data && body.data.valid) {
			console.log(chalk`{green yes} ${username}`);
			return { type: "yes", username, info: body.data.valid };
		} else {
			console.log(chalk`{red yok} ${username}`);
			return { type: "yok", username };
		}
	}
}
